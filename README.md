...
> **部署时不需要填写任何密钥**——部署完成后打开 `/admin`，首次访问会让你创建管理员账号，
> 短信商密钥和人机检测密钥都在管理后台上传填写（左侧菜单「短信通道」「人机检测」）。详见 [一键部署说明.md](./一键部署说明.md)。

给落地页加一道「人机检测 → 短信验证码 → 才跳转 WhatsApp」的关卡。
一个 Cloudflare Worker 同时提供：三个短信通道（Twilio / Plivo / Onbuka）、频控、验证记录、免验证凭证、网页管理后台。
...
2. 左侧菜单「短信通道」页底部发一条测试短信，确认通道显示 `twilio` 且能收到短信；
3. 打开 `/demo` 走一遍完整流程；
...
3. Site Key 填到后台「人机检测」，Secret Key 用
...
| | Twilio Verify | Plivo Verify | Onbuka |
|---|---|---|---|
| 类型 | OTP 服务（码由服务商管） | OTP 服务（码由服务商管） | 普通短信网关（**码由本服务自己生成校验**） |
| 开通方式 | 自助注册，即时可用 | **需联系销售，自助不可用** | 联系销售开通 |
| 门槛 | 无最低消费 | **最低月消费承诺 $1,000 起** | 按量计费 |
| 费用（美国） | $0.05/成功验证 + 约 $0.0083/条 | 只收短信费（约 $0.0077/条） | 按套餐，通常更便宜 |
| 美国 10DLC | 仅做用户验证时可免注册 | 免注册 | 需按其要求报备 |
| 需要填的凭证 | Account SID / Auth Token / Service SID | Auth ID / Auth Token / App UUID | Api-Key / API Secret / 应用 ID |

三家都已在后台「短信通道」页提供填写入口，**改完即时生效，不需要重新部署**。建议：

- 想最快跑通、最省心 → **Twilio Verify**（自助注册、文档最全）；
- 追求低成本、已有 Onbuka 账号 → **Onbuka**（本服务会自动生成并校验验证码，有效期 10 分钟）；
- 用量大到能覆盖 Plivo 的最低消费 → 再切 **Plivo**。

在后台把「短信通道」设为 `auto` 并排好优先级，前一家失败会自动切下一家。

### Onbuka 的特殊说明

Onbuka 只有「发短信」接口，没有 OTP 校验接口，所以本服务承担了验证码的生成与校验：

- 验证码 6 位随机、**哈希 + 盐**存库（数据库里查不到明文），有效期 10 分钟；
- 短信内容用后台的「短信模板」控制，支持 `{code}`（验证码）和 `{brand}`（品牌名）占位符；
- 签名按官方文档实现：`Sign = md5(Api-Key + API Secret + 秒级时间戳)`，请求头带 `Sign` / `Timestamp` / `Api-Key`；
- 号码会去掉 `+` 前缀后提交（Onbuka 要求纯数字）。

## 五、后台功能（`/admin`）

后台是**左侧菜单 + 右侧内容**的常规后台布局，左下角一键切换**白天 / 黑夜**主题（默认白天）。
菜单按用途分成四组，不用在一页里翻来翻去：

**数据**

- **概览**：30 天发送量、验证通过率、失败数、跳转数、近 1 小时量、按国家/通道/站点拆分、14 天趋势；
- **验证记录**：按手机号 / 状态 / 通道 / 站点 / 日期筛选，分页浏览，一键导出 CSV。

**短信**

- **短信通道**：
  - 通道选择：`auto`（按优先级容灾）/ 仅 Twilio / 仅 Plivo / 仅 Onbuka / mock；
  - 优先级：auto 模式下的尝试顺序（可排三家），前一个失败自动切下一个；
  - **通道凭证**：Twilio / Plivo / Onbuka 的密钥，**加密存你自己的 D1**，保存即生效；
  - 运行模式：调试模式开关（开启后不发真实短信，验证码固定 `123456`）；
  - 页面底部「通道状态与测试」：给任意号码发一条测试短信，直接看到走了哪个通道、失败原因是什么，并显示各项密钥是否已配置（自建验证码通道会把验证码一并回显）。
- **人机检测**：Turnstile 开关 + Site Key + Secret Key；
- **国家与频控**：
  - 允许的国家：勾一个国家 = 只允许该国，勾多个 = 多国放行；有预设按钮（仅美国 / 美加 / 英语区 / 欧美）；
  - 频率限制：重发间隔（默认 60 秒）、单号码每日上限（5）、单 IP 每小时上限（30）、全站每小时上限（2000，熔断保护）、单次最多试错次数（5）、免验时长（验证通过后多少小时内免二次验证，0 = 每次都验证）。

**落地页**

- **跳转与站点**：链接模式（`client` 链接留在落地页 / `server` 验证通过后由后台下发）、轮询策略、允许的站点来源白名单（`*` = 不限制，**生产建议填具体域名**）、弹窗主题色；
- **链接池**：后台统一管理 WhatsApp 链接（按站点、开关、权重、每 24 小时上限，带统计，见下一节）；
- **弹窗文案**：验证弹窗上的所有文字（标题、按钮、错误提示等）都在这里改。

**系统**

- **我的账号**：修改后台密码、查看管理员列表。

配置改完点右上角 **保存修改**，约 10 秒对所有落地页生效，**不需要重新部署**。

### 链接池（`server` 模式下由后台统一管链接）

「链接池」只对 `server` 模式生效：验证通过后由后台挑一条链接下发，落地页源码里看不到真实链接。

- **按站点分池**：每条链接归属一个站点（值就是落地页 `SMS_VERIFY_CONFIG` 里的 `site`），也可以放进**通用池**给所有站点共享；
- **分配顺序**：站点专属池 → 通用池 → 两个池都没得发时（没配 / 全停用 / 到上限），自动回落到落地页自带的链接，**不会丢转化**；
- **轮询策略**（在「跳转与站点」页选）：`加权随机`（默认）/ `纯随机` / `最闲优先`（按权重折算已分配次数）；
- **每条链接独立控制**：启用开关、权重、**每 24 小时分配上限**（0 = 不限，防止某个群被一次性塞爆）；
- **统计**：近 24 小时分配 / 累计分配 / 累计跳转；「验证记录」里也能看到这次用的是哪条链接；
- **批量导入**：一行一条粘贴即可，自动去重、自动跳过无效链接。

同一个用户 24 小时内（免验期内）再点按钮，会**保持同一条链接**，不会每次换群；只有那条链接被停用或删掉才会重新分配。
...
| 一直收不到短信 | 后台「短信通道」页发一条测试看错误原因；Twilio 试用账号只能发给已验证号码；确认目标国家是否在通道覆盖内 |
...
└── worker/
    ...
    ├── schema.sql              # 完整建表脚本（网页后台手动部署时粘贴这个）
    ├── migrations/
    │   ├── 0001_init.sql       # D1 建表脚本（部署时自动执行）
    │   ├── 0002_secrets_and_code.sql
    │   └── 0003_links.sql      # 链接池表
    ...
    └── src/
        ...
        │   ├── db.js           # 记录、频控、统计、清理
        │   ├── links.js        # WhatsApp 链接池：增删改查 + 分配算法
        │   ├── auth.js         # 后台登录与会话
