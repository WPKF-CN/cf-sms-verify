...
> 短信商密钥和人机检测密钥都在后台「配置」页填写。详见 [一键部署说明.md](./一键部署说明.md)。
...
三家都已在后台「配置」页提供填写入口，**改完即时生效，不需要重新部署**。建议：
...
- **配置**（保存后约 10 秒对所有落地页生效，无需重新部署）：
  ...
  | 短信通道 | `auto`（按优先级容灾）/ 仅 Twilio / 仅 Plivo / 仅 Onbuka / mock |
  | 优先级 | auto 模式下的尝试顺序（可排三家），前一个失败自动切下一个 |
  | **短信通道凭证** | Twilio / Plivo / Onbuka 的密钥，**加密存 D1**，改完即时生效 |
  | **Turnstile Secret** | 人机检测密钥，同样在后台填写 |
...
- **测试**：给任意号码发一条测试短信，直接看到走了哪个通道、失败原因是什么；同时显示各通道密钥是否已配置。自建验证码通道会把验证码一并回显，方便核对。
